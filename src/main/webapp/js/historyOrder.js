    ...
    fetch(url)
            ...
            .then(data => {
                ...
                if (data.success) {
                    ...
                    showToast('Unable to retrieve order information', 'error');
                }
            })
            .catch(error => {
                console.error('Error fetching order details:', error);
                showToast('An error occurred while retrieving order information', 'error');
            });
}
...
function submitEditOrder(form) {
    ...
    resultDiv.innerHTML = `<p style="color:blue; font-weight:bold;">Processing update request...</p>`;
    ...
    fetch(form.action, {
        ...
        body: params,
            ...
            .then(text => {
                ...
                if (data.success) {
                    resultDiv.innerHTML = `<p style="color:green; font-weight:bold;">${data.message || "Order updated successfully!"}</p>`;

                    setTimeout(() => {
                        closeModal("editOrderModal");
                        window.location.reload(); // Reload page to display new data
                    }, 800);
                } else {
                    resultDiv.innerHTML = `<p style="color:red; font-weight:bold;">Update failed: ${data.message}</p>`;
                }
            })
            .catch(error => {
                console.error("Error:", error);
                resultDiv.innerHTML = `<p style="color:red; font-weight:bold;">Error updating order: ${error.message}</p>`;
            });
...
/**
 * View grouped order details (all products in an order with the same referral code)
 * @param {string} referralCode - Referral code of the order group to view details for
 */
function viewGroupedOrderDetails(referralCode) {
    // Build the correct URL format
    const baseUrl = window.location.origin;
    const pathArray = window.location.pathname.split('/');
    const contextPath = pathArray[1] ? '/' + pathArray[1] : '';
    
    console.log("Viewing grouped details for referralCode:", referralCode);
    
    // Ensure full and accurate URL path
    const url = `${baseUrl}${contextPath}/historyorder/groupdetails?code=${encodeURIComponent(referralCode)}`;
    console.log("Requesting grouped order details from URL:", url);
    ...
    fetch(url)
        ...
        .then(data => {
            console.log("Grouped order details received:", data);
            ...
            if (data.success) {
                ...
                document.getElementById('modal-order-date').textContent = formatDate(orderSummary.orderDate || '');
                document.getElementById('modal-order-code').textContent = orderSummary.referralCode || '';
                
                // Customer information
                document.getElementById('modal-customer-name').textContent = orderSummary.customerName || '';
                document.getElementById('modal-customer-phone').textContent = orderSummary.customerPhone || '';
                document.getElementById('modal-customer-address').textContent = orderSummary.shippingAddress || '';
                
                // Clear and populate products container
                ...
                orderItems.forEach((item, index) => {
                    ...
                    productDiv.className = 'order-product-details';
                    ...
                    const imageUrl = item.imageId ? 
                        `${contextPath}/ImagesServlet?type=product&imageId=${item.imageId}` : 
                        `${contextPath}/img/default-product.jpg`;
                    
                    productDiv.innerHTML = `
                        <div class="product-item ${index > 0 ? 'product-separator' : ''}">
                            <img src="${imageUrl}" alt="${item.productName || 'Product'}" class="product-image">
                            <div class="product-details">
                                <h3>${item.productName || 'Unknown Product'}</h3>
                                <p><strong>Unit Price:</strong> ${item.formattedUnitPrice || formatCurrency(item.unitPrice || 0)}</p>
                                <p><strong>Quantity:</strong> ${item.quantity || 1}</p>
                                <p><strong>Subtotal:</strong> ${item.formattedSubtotal || formatCurrency(subtotal)}</p>
                            </div>
                        </div>
                    `;
                ...
                // Payment details - Original Price (sum of all subtotals)
                document.getElementById('modal-payment-subtotal').textContent = formatCurrency(totalOriginalPrice);
                
                // Always use totalAmount for the final price
                const finalPrice = orderSummary.totalAmount || 0;
                document.getElementById('modal-total-amount').textContent = orderSummary.formattedTotalAmount || formatCurrency(finalPrice);
                
                // Calculate the discount voucher amount (original price - final price)
                const discountVoucher = totalOriginalPrice - finalPrice || 0;
                document.getElementById('modal-discountVoucher').textContent = discountVoucher > 0 ? '-' + formatCurrency(discountVoucher) : formatCurrency(0);
                
                // Voucher information - for grouped orders, we'll use the first item's voucher info
                const voucherContainer = document.getElementById('voucher-info-container');
                const firstItem = orderItems[0];
                
                if (firstItem && firstItem.voucherCode && firstItem.voucherCode !== 'No voucher applied') {
                    // Display voucher details
                    document.getElementById('modal-voucher-code').textContent = firstItem.voucherCode || 'N/A';
                    
                    // Format discount amount with negative sign to show reduction
                    const discountAmount = firstItem.discountAmount || 0;
                    document.getElementById('modal-discount-amount').textContent = '-' + formatCurrency(discountAmount);
                    
                    voucherContainer.style.display = 'block';
                } else {
                    // If no voucher, hide the voucher details
                    voucherContainer.style.display = 'none';
                }

...
public class CartServlet extends HttpServlet {
    ...
    private void addToCart(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        try {
            ...
            System.out.println("Bắt đầu addOrUpdate cho customerId: " + customerId + ", productId: " + productId);
            ...
            e.printStackTrace();
            response.getWriter().write("{\"status\":\"error\", \"message\":\"Lỗi khi thêm sản phẩm: " + e.getMessage() + "\"}");
        }
    }
    ...
    private void viewCart(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            System.out.println("ViewCart: customerId=" + customerId + ", cartCount=" + totalItems);
    ...
    private void removeFromCart(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        if (customerId == -1) {
            ...
            jsonResponse.put("message", "Bạn cần đăng nhập để xóa sản phẩm!");
            response.getWriter().write(jsonResponse.toString());
            return;
        }
    ...
    private void clearCart(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        if (customerId == -1) {
            jsonResponse.put("status", "error");
            jsonResponse.put("message", "Bạn cần đăng nhập để xóa giỏ hàng!");
            response.getWriter().write(jsonResponse.toString());
            return;
        }
        ...
        try {
            ...
            jsonResponse.put("status", "success");
            jsonResponse.put("cartCount", 0);
            jsonResponse.put("cartTotal", 0.0);
            response.getWriter().write(jsonResponse.toString());
        } catch (SQLException e) {
            e.printStackTrace();
            jsonResponse.put("status", "error");
            jsonResponse.put("message", "Lỗi khi xóa giỏ hàng: " + e.getMessage());
            response.getWriter().write(jsonResponse.toString());
        }
    }
